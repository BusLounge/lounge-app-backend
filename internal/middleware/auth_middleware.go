...
func AuthMiddleware(jwtService *jwt.Service) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if len(parts) != 2 || parts[0] != "Bearer" {
			headerSnippet := authHeader
			if len(headerSnippet) > 20 {
				headerSnippet = headerSnippet[:20]
			}
			log.Printf("AUTH FAILED: Invalid auth format - Header: %s, Path: %s, IP: %s", headerSnippet, c.Request.URL.Path, c.ClientIP())
			c.JSON(http.StatusUnauthorized, gin.H{
				"error":   "unauthorized",
